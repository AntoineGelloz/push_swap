...
/*   Updated: 2019/09/03 17:22:48 by agelloz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_is_to_push(t_stack *stack, t_s *s, int elements)
{
	...
	{
		if (!ft_is_pair(elements + s->pushed) && *(int *)stack->head->content > s->median)
			return (1);
		else if (ft_is_pair(elements + s->pushed) && *(int *)stack->head->content >= s->median)
			return (1);
	}
	return (0);
}
...
t_list	*ft_list_index(t_list *list, int index)
{
	while (list && index--)
		list = list->next;
	return (list);
...
int	ft_unsorted_top(t_stack *stack, t_s *s, int elements)
{
	...
	if (stack->name == 'a')
		ft_lstrev(&head1);
	sorted_copy = ft_list_index(head1, stack->elements - elements);
	head2 = ft_lstcpy(stack->head, copy_values);
	rev = ft_list_index(head2, stack->elements - elements);
	i = -1;
...
int	ft_is_list_sorted(t_list *list, int elements, int (*ft_order)(void *, void *))
{
	...
	if (ft_order(list->content, list->next->content) > 0)
		return (ft_is_sorted(list->next, elements - 1, ft_order));
	return (0);
}
